...
import { GetCommandFromS, Convert, IsIdOwner, OwnerToUserArray } from "../m/func";
...
module.exports = async function run(client :Client, message : Message) : Promise<void> {
    ...
    if (cmd.Owner) if(! IsIdOwner(message.author.id) ) {message.channel.send(`Sorry, but the command you tried to execute requires you to be an "Owner", Here's a list of them \` ${(await OwnerToUserArray(client)).map(v => v.tag).join(',')} \` `);return}
